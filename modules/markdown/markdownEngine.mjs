...
function isAsciiTrimmable(c2) {
  return c2 === 32 || c2 === 9 || c2 === 10 || c2 === 13;
}
...
function applyReplacements(str, replacements) {
  let result = "";
  let lastPos = 0;
  replacements.sort((a2, b2) => a2.pos - b2.pos);
...
LinkifyIt.prototype.match = function match(text2) {
  ...
  function choose(a2, b2) {
    ...
    if (a2.index !== b2.index) {
      return a2.index < b2.index ? a2 : b2;
    }
    return a2.lastIndex >= b2.lastIndex ? a2 : b2;
  }
  if (!text2.length) {
    return null;
  }
...
// scripts/discovery-md-bundle-entry.mjs
...
function githubLikeSlugify(str) {
  return String(str ?? "").trim().toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^\p{L}\p{N}\s-]/gu, "").replace(/\s+/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");
}
function highlightCode(str, lang) {
  const code2 = String(str ?? "");
  ...
  try {
    return common_default.highlightAuto(code2).value;
  } catch {
...
function createMarkdownIt() {
  ...
  md.use(import_markdown_it_task_lists.default, { enabled: true, label: true });
  md.use(emoji_plugin2);
  md.use(b, {
    level: [1, 2, 3, 4, 5, 6],
    tabIndex: false,
    slugify: githubLikeSlugify,
    permalink: b.permalink.linkInsideHeader({
      symbol: "\xB6",
      placement: "before",
      class: "nx-md-header-anchor",
      ariaHidden: true
    })
  });
  ...
  md.renderer.rules.fence = (tokens, idx, options, env, self) => {
    const token = tokens[idx];
    const lang = String(token.info || "").trim().split(/\s+/g)[0];
    if (lang === "mermaid") {
...
function renderMarkdown(src) {
  if (!sharedInstance) sharedInstance = createMarkdownIt();
  return sharedInstance.render(String(src ?? ""));
}
var renderDiscoveryMarkdown = renderMarkdown;
export {
  createMarkdownIt,
  githubLikeSlugify,
