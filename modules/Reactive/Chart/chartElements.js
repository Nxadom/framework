...
class chartElements {
  ...
  createChartConfigModal(chartType, modalId) {
          ...
          <div>
            ...
            <div class="nx-row">
              <div class="nx-col-6">
                <div class="form-group">
                  <dl>
                    <dt class="form-group-header"><label for="chartWidth-${modalId}">Width (px)</label></dt>
                    <dd class="form-group-body">
    ...
    this.nexaUI.modalHTML({
      ...
      footer: `
        <button type="button" class="btn" onclick="if(typeof previewChart_${modalId} === 'function') { previewChart_${modalId}(); } else { console.error('Function previewChart_${modalId} not found!'); }" style="margin-right: 0.5rem;">
          🔍 Preview
        </button>
        <script>
  ...
  insertChartAtPosition(element) {
    try {
      ...
      if (!targetElement || !targetElement.parentNode) {
        console.warn("No valid target for chart insertion");
