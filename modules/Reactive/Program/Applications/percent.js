...
console.log('data:', data);
 ...
 // const Federated= new NXUI.Federated(data);
 // const ass= await Federated.analysis(natKey);
 // console.log('ass:', ass);
...
const Federated= new NXUI.Federated({
 ...
 const dataTabel= await Federated.analysis(data.failed);
    ...
    // Create Narasi instance with proper interactions object
    const interactions = {
      config: data.config || {},
      nexaUI: data.nexaUI || window.NXUI
    };
    const narasi = new Narasi(interactions);
    




    // Buat HTML konten dengan text standar
    const contentHTML = narasi.createNarasiFromData(dataTabel);
...
export class Narasi {
  ...
  generateContentFromData(dataTabel) {
    ...
    let content = `
      ...
      <div style="margin-bottom: 25px; padding: 15px; background: #f8f9fa; border-left: none; border-radius: 4px; text-align: center;">
    ...
    if (dataTabel.response && Array.isArray(dataTabel.response) && dataTabel.response.length > 0) {
      ...
      content += `
        <div style="margin-bottom: 25px;">
          ...
          <div style="overflow-x: auto;">
            <table style="width: 100%; border-collapse: collapse; font-size: 13px; background: white; border: none;">
              <thead>
                <tr style="background: #2c3e50; color: white;">
                  <th style="padding: 12px 15px; text-align: center; font-weight: 600; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; border: none; width: 60px;">
                    NO
                  </th>
      `;
      ...
      keys.forEach(key => {
        ...
        content += `
          <th style="padding: 12px 15px; text-align: ${textAlign}; font-weight: 600; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px; border: none;">
            ${key.replace(/_/g, ' ')}
          </th>
        `;
      });
      ...
      dataTabel.response.forEach((item, index) => {
        ...
        // Add No column
        content += `
          <td style="padding: 12px 15px; color: #2c3e50; border: none; text-align: center; font-weight: 600;">
            ${index + 1}
          </td>
        `;
        ...
        keys.forEach(key => {
          ...
          const textAlign = isTotal ? 'center' : 'left';
          content += `
            <td style="padding: 12px 15px; color: #2c3e50; border: none; text-align: ${textAlign}; font-weight: 600;">
              ${value}
            </td>
          `;
        });
  ...
  createNarasi(
    ...
    const narasiHTML = `
      ...
      ">
        <!-- Document Header - White Gray Style -->
