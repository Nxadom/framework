...
import { iconElements } from "../Icon/iconElements.js";
import { GridSystem } from "../Grind/GridSystem.js";
import { NexaCmirror } from "../../codemirror/NexaCmirror.js";

class Elements {
  constructor(data = null) {
    this.data = data;
    // Initialize NexaUI instance using consistent pattern from modal.html
    this.nexaUI = null;
    ...
    this.borderRadius = new BorderRadius(this); // Initialize BorderRadius instance
    this.shadowBox = new ShadowBox(this); // Initialize ShadowBox instance
    this.iconElements = new iconElements(this); // Initialize iconElements instance
    this.gridSystem = new GridSystem(this); // Initialize GridSystem instance
    this.currentTargetElement = null; // Store reference to current target element being edited

    // Grid system configuration properties
    this.gridSnapEnabled = true; // Enable grid snapping by default
    this.gridSnapSize = 20; // Default snap size in pixels
    this.targetElement = null; // For GridSystem compatibility

    // Setup global functions for modal integration
    this.setupGlobalFunctions();

    // Initialize NexaUI after setup
    this.initializeNexaUI();
  }

  /**
   * Initialize NexaUI instance following modal.html pattern
   */
  initializeNexaUI() {
    try {
      this.nexaUI = NexaUI();
      console.log("✅ NexaUI initialized successfully");
    } catch (error) {
      console.error("❌ Failed to initialize NexaUI:", error);
      this.nexaUI = null;
    }
  }

  /**
   * Close modal using simple pattern like modal.html
   * @param {String} modalId - Modal ID to close
   */
  closeModal(modalId) {
    if (window.nexaUI && window.nexaUI.nexaModal) {
      window.nexaUI.nexaModal.close(modalId);
    } else if (typeof NexaUI === "function") {
      const nexaUI = NexaUI();
      nexaUI.nexaModal.close(modalId);
    } else {
      console.error("NexaUI not available for modal close");
    }
  }
  struktur() {
    const menuStructure = [
  ...
  /**
   * Initialize modal dialogs following modal.html pattern
   */
  initModals(data) {
    // Check if NexaUI is available
    if (!this.nexaUI) {
      console.error("❌ NexaUI not initialized");
      return;
    }

    // Create fresh modal with new data - following modal.html pattern
  ...
  setupGlobalFunctions() {
    ...
    // Store original closeModal if exists
    const originalCloseModal = window.closeModal;

    // Override closeModal to handle preview mode cleanup - following modal.html pattern
    window.closeModal = function (modalId) {
      console.log("🚪 Close modal called for:", modalId);

      // Check if this is an Elements edit modal and cleanup preview mode
      if (modalId && modalId.includes("elementsEditFormModal")) {
        console.log("🎭 Cleaning up Elements preview mode for:", modalId);

        // Clean up preview mode first
        if (elementsInstance.currentTargetElement) {
          // Remove preview mode indicator
          if (
            elementsInstance.formGenerator &&
            typeof elementsInstance.formGenerator.removePreviewModeIndicator ===
              "function"
          ) {
            elementsInstance.formGenerator.removePreviewModeIndicator(
              elementsInstance.currentTargetElement
            );
            console.log("🎭 Preview mode indicator removed");
          }

          // Restore original state if available
          if (
            elementsInstance.formGenerator &&
            elementsInstance.formGenerator.originalElementState
          ) {
            console.log("🔄 Restoring element to original state...");
            elementsInstance.formGenerator.restoreOriginalElementState(
              elementsInstance.currentTargetElement
            );
          }

          // Cleanup form generator
          if (
            elementsInstance.formGenerator &&
            typeof elementsInstance.formGenerator.cleanup === "function"
          ) {
            elementsInstance.formGenerator.cleanup();
          }

          // Clear reference
          elementsInstance.currentTargetElement = null;
        }

        // Close modal using nexaUI
        elementsInstance.nexaUI.nexaModal.close(modalId);
      } else {
        // Regular close modal like modal.html
    ...
    window.processElementsBorderRadius = function (modalId, data) {
      elementsInstance.borderRadius.processElementsBorderRadius(modalId, data);
    };

    // Add shadow box handler
    window.processElementsShadowBox = function (modalId, data) {
      elementsInstance.shadowBox.processElementsShadowBox(modalId, data);
    };

    // Add icon elements handler
    window.processElementsIcon = function (modalId, data) {
      elementsInstance.iconElements.processElementsIcon(modalId, data);
    };

    // Add code mirror handler
    window.processElementsCodeMirror = function (modalId, data) {
      elementsInstance.processElementsCodeMirror(modalId, data);
    };

    // Add text formatting global functions if needed
    // TextFormatting class handles its own modal functions internally
  }
  ...
  /**
   * Process elements edit data - following modal.html pattern
   */
  processElementsEdit(modalId, data = null) {
    ...
    // Close modal - using consistent helper method
    this.closeModal(modalId);
  }

  /**
   * Process elements deletion - following modal.html pattern
   */
  processElementsDelete(modalId, data = null) {
    ...
    this.currentTargetElement.parentNode.removeChild(this.currentTargetElement);
    ...
    // Close modal and cleanup - using consistent helper method
    this.closeModal(modalId);
  ...
  processElementsFormCancel(modalId, data = null) {
    ...
    try {
      ...
      if (!this.currentTargetElement) {
        ...
        // ALWAYS remove preview mode indicator first
        if (
          this.formGenerator &&
          typeof this.formGenerator.removePreviewModeIndicator === "function"
        ) {
        ...
        // Then restore original state if available
        if (this.formGenerator.originalElementState) {
          console.log("🔄 Restoring element to original state...");
          const restoredElement =
            this.formGenerator.restoreOriginalElementState(
              this.currentTargetElement
            );
  ...
  async elementsCode(data = null) {
    if (!data?.targetElement) {
      console.warn("No target element provided for code mirror");
      return;
    }
    ...
    // Get HTML content from target element
    const elementHTML = data.targetElement.outerHTML || data.targetElement.innerHTML || "";

    // Create modal for Code Mirror
    const modalId = "elementsCodeMirrorModal";
    const editorId = "codeEditor_" + Date.now();
    const btnSaveId = "btnSave_" + Date.now();
    const btnCopyId = "btnCopy_" + Date.now();
    
    // Escape HTML untuk template literal (handle backticks dan ${})
    const escapedHTML = elementHTML
      .replace(/\\/g, '\\\\')
      .replace(/`/g, '\\`')
      .replace(/\${/g, '\\${');
    
    // Create modal structure with NexaCmirror structure
    this.initModals({
      elementById: modalId,
      styleClass: "w-1000px code-mirror-modal",
      label: "Code Mirror",
      onclick: {
        title: "Save",
        cancel: "Cancel",
        send: "processElementsCodeMirror",
        titleClass: "nx-btn-primary",
        cancelClass: "nx-btn-secondary",
      },
      content: `
        <div class="NexaCmirror-editorContainer" style="opacity: 0; visibility: hidden;">
          <div class="nx-card-header" style="display: none;">
            <h3 class="bold">
              <i class="fas fa-code" id="modeIcon"></i>
              <span id="modeText">Nexa Mirror</span>
            </h3>
            <div class="NexaCmirror-editorToolbar">
              <button id="${btnSaveId}" class="NexaCmirror-btn-save" title="Save">
                <span class="material-symbols-outlined">save</span>
              </button>
            </div>
          </div>
          <div class="nx-card-body nx-scroll" style="padding: 0px; height: 500px;">
            <textarea id="${editorId}">${escapedHTML}</textarea>
          </div>
        </div>
      `,
      footer:`
       <button id="${btnCopyId}" class="NexaCmirror-btn-copy" title="Copy">
    ...
    // Wait for modal to be visible, then initialize editor
    setTimeout(async () => {
      try {
        ...
        if (!textareaElement) {
          console.error("❌ Textarea element not found:", editorId);
          return;
        }

        // Initialize NexaCmirror editor
        const editor = new NexaCmirror(editorId, {
          mode: 'htmlmixed',
          height: '100%',
          save: btnSaveId,
          copy: btnCopyId,
          ...
          status: true,
          onSave: (code) => {
            // Update target element with new code
            if (this.currentTargetElement) {
              try {
                ...
                // If there's only one root element, replace the target with it
                if (tempDiv.children.length === 1) {
                  const newElement = tempDiv.children[0];
                  this.currentTargetElement.replaceWith(newElement);
                  this.currentTargetElement = newElement;
                } else {
                  // If multiple elements, replace innerHTML
                  this.currentTargetElement.outerHTML = code;
                }
              } catch (error) {
        ...
        console.error("❌ Error initializing NexaCmirror:", error);
      }
    }, 300); // Wait 300ms for modal to be fully rendered
  }

  /**
   * Process Code Mirror save action
   * Called when user clicks Save button in modal
   */
  processElementsCodeMirror(modalId, data = null) {
    try {
      let code = "";
      
      // Try to get code from editor instance first
      if (this.codeMirrorEditor) {
        // Try getValue method from NexaCmirror instance
        if (typeof this.codeMirrorEditor.getValue === 'function') {
          code = this.codeMirrorEditor.getValue();
        } 
        // Try getEditor method to access CodeMirror directly
        else if (typeof this.codeMirrorEditor.getEditor === 'function') {
          const cmEditor = this.codeMirrorEditor.getEditor();
      ...
      // Fallback: try to get from CodeMirror directly via DOM
      if (!code) {
        const modal = document.getElementById(modalId);
        if (modal) {
          const textarea = modal.querySelector('textarea[id^="codeEditor_"]');
          if (textarea) {
            // CodeMirror.fromTextArea stores instance in textarea.__CodeMirror
            if (textarea.__CodeMirror && typeof textarea.__CodeMirror.getValue === 'function') {
              code = textarea.__CodeMirror.getValue();
            }
            // Or check nextElementSibling (CodeMirror wrapper)
            else if (textarea.nextElementSibling) {
              const wrapper = textarea.nextElementSibling;
              if (wrapper.CodeMirror && typeof wrapper.CodeMirror.getValue === 'function') {
                code = wrapper.CodeMirror.getValue();
              }
            }
            // Last resort: get from textarea value directly
      ...
      if (!code) {
        console.warn("⚠️ No code available from editor");
        this.closeModal(modalId);
        return;
      }
      ...
      if (this.currentTargetElement) {
        try {
          ...
          if (tempDiv.children.length === 1) {
            ...
            // If multiple elements, replace outerHTML
            this.currentTargetElement.outerHTML = code;
          }
        } catch (error) {
          console.error("❌ Error updating element:", error);
        ...
        console.warn("⚠️ No target element available for update");
      }
      ...
      console.error("❌ Error processing Code Mirror save:", error);
      this.closeModal(modalId);
    }
  }

  /**
   * Helper method to escape HTML for textarea
   */
  escapeHtml(text) {
  ...
  elementsBorderRadius(data = null) {
    // Delegate to BorderRadius class
    this.borderRadius.elementsBorderRadius(data);
  }

  elementsShadowBox(data = null) {
    // Delegate to ShadowBox class
    this.shadowBox.elementsShadowBox(data);
  }

  elementsIcon(data = null) {
    // Delegate to iconElements class
    this.iconElements.elementsIcon(data);
  }

  // Grid System Actions
  gridElement(data = null) {
    console.log("🔲 Grid Element action triggered", data);
    // Update target element if provided
    if (data?.targetElement) {
      this.currentTargetElement = data.targetElement;
      this.targetElement = data.targetElement;
    }
    // This is the main grid action, could show a general grid modal or info
  }

  /**
   * Update target element for grid operations
   */
  setTargetElementForGrid(data = null) {
    if (data?.targetElement) {
      this.currentTargetElement = data.targetElement;
      this.targetElement = data.targetElement;
      console.log(
        "🎯 Target element updated for grid operations:",
        data.targetElement.tagName
      );
    }
  }

  addGridColumn1(data = null) {
    console.log("📐 Adding 1 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(1);
  }

  addGridColumn2(data = null) {
    console.log("📐 Adding 2 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(2);
  }

  addGridColumn3(data = null) {
    console.log("📐 Adding 3 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(3);
  }

  addGridColumn4(data = null) {
    console.log("📐 Adding 4 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(4);
  }

  addGridColumn5(data = null) {
    console.log("📐 Adding 5 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(5);
  }

  addGridColumn6(data = null) {
    console.log("📐 Adding 6 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(6);
  }

  addGridColumn7(data = null) {
    console.log("📐 Adding 7 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(7);
  }

  addGridColumn8(data = null) {
    console.log("📐 Adding 8 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(8);
  }

  addGridColumn9(data = null) {
    console.log("📐 Adding 9 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(9);
  }

  addGridColumn10(data = null) {
    console.log("📐 Adding 10 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(10);
  }

  addGridColumn11(data = null) {
    console.log("📐 Adding 11 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(11);
  }

  addGridColumn12(data = null) {
    console.log("📐 Adding 12 column grid", data);
    this.setTargetElementForGrid(data);
    return this.gridSystem.contextAddGridColumn(12);
  }

  /**
   * Insert element at current position (required by GridSystem)
   */
  insertElementAtPosition(element) {
    try {
      // Check for target element (use currentTargetElement or targetElement)
      const targetElement = this.currentTargetElement || this.targetElement;

      console.log("🔍 insertElementAtPosition Debug Info:", {
        hasCurrentTarget: !!this.currentTargetElement,
        hasTargetElement: !!this.targetElement,
        finalTarget: targetElement?.tagName,
        elementToInsert: element?.tagName,
        elementClasses: element?.className,
      });

      if (!targetElement) {
        console.warn("⚠️ No target element available for grid insertion");

        // Fallback: append to body if no target element
        if (document.body) {
          document.body.appendChild(element);
          console.log("✅ Grid element appended to body (fallback)");
          return { success: true, message: "Grid element added to page" };
        }

        return {
          success: false,
          error: "No target element and no body available",
        };
      }
      ...
      console.log("🎯 Target element info:", {
        ...
        id: targetElement.id || "(no id)",
        classes: targetElement.className || "(no classes)",
        hasParent: !!targetElement.parentNode,
        parentTag: targetElement.parentNode?.tagName,
      });

      // Ensure target element has a parent
      if (!targetElement.parentNode) {
        console.warn("⚠️ Target element has no parent, appending to body");
        document.body.appendChild(element);
        console.log("✅ Grid element appended to body (no parent)");
        return { success: true, message: "Grid element added to page" };
      }
      ...
      setTimeout(() => {
        ...
        if (insertedElement) {
          console.log("✅ Grid element verified in DOM:", {
            found: true,
            position: insertedElement.getBoundingClientRect(),
            parent: insertedElement.parentNode.tagName,
            visible:
              insertedElement.offsetHeight > 0 &&
              insertedElement.offsetWidth > 0,
          });
        } else {
          console.warn("⚠️ Grid element not found in DOM after insertion");
        }
      }, 100);

      console.log("✅ Grid element inserted successfully after target element");
      return { success: true, message: "Grid element added successfully" };
    } catch (error) {
      console.error("❌ Error inserting grid element:", error);

      // Final fallback: try to append to body
      try {
        if (document.body) {
          document.body.appendChild(element);
          console.log("✅ Grid element appended to body (error recovery)");
          return {
            success: true,
            message: "Grid element added (recovered from error)",
          };
        }
      } catch (fallbackError) {
  ...
  // BorderRadius form generation moved to BorderRadius.js

  removedGenerateBorderRadiusForm(currentValues) {
    return `
      <div class="form-nexa">
  ...
  // extractNumericValue moved to BorderRadius.js

  // processElementsBorderRadius moved to BorderRadius.js

  removedProcessElementsBorderRadius(modalId, data = null) {
    // This method has been moved to BorderRadius.js
    console.warn(
      "⚠️ processElementsBorderRadius has been moved to BorderRadius class"
    );
    return;
  ...
  removedShowNotification(message, type = "info") {
    ...
    // Notification system removed - check console for messages
  }

  // createVisualNotification method removed
  removedCreateVisualNotification(message, type, icon) {
