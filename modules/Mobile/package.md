...
```javascript
import { View, Text, Buttons, Colors } from "Nxdom";
...
```javascript
import { Input, useFormValidation, Buttons } from "Nxdom";
...
```javascript
import { ImgPicker, pickImage, Avatar } from "Nxdom";
...
```javascript
import { NexaFirestore, initNexaFirebase } from "Nxdom";
...
```javascript
import { RichTextEditor, View } from "Nxdom";
...
```javascript
import { Buttons, ButtonsAction, isBtnGroup, BtnTabs, Loader } from "Nxdom";
...
```javascript
import { QRCodeGenerator, ExpoSpeech, View } from "Nxdom";

export default function QRSpeechExample() {
  const speakText = async () => {
    await ExpoSpeech.speak("Hello from Nxdom!");
  };
...
```javascript
import { Grid, View, Text, Colors } from "Nxdom";
...
```javascript
import { useModal, fs, FontFamily, View, Text, Button } from "Nxdom";
...
```javascript
import { NexaSync, createNexaSync, AsyncStorage } from "Nxdom";
