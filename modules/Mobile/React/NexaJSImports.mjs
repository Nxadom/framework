...
const allowedSpecifier = "Nxdom";
const legacySpecifiers = new Set(["nexajs", "nexaui"]);
...
function shouldCheckSpecifier(specifier) {
  ...
  return normalized === "nxdom" || legacySpecifiers.has(normalized);
}
...
if (findings.length > 0) {
  console.error('\nInvalid Nxdom import found. Use exactly "Nxdom".\n');
  for (const item of findings) {
    console.error(`- ${item.filePath}: ${item.specifiers.join(", ")}`);
  }
  console.error('\nExample valid import:\nimport { View } from "Nxdom";\n');
  process.exit(1);
}

console.log('Nxdom import validation passed: only "Nxdom" is used.');
