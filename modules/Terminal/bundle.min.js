...
export class CommandRow {
    ...
    constructor(commandLine, onEnterCallback) {
        ...
        this.commandTime.innerHTML = ``;

        this.commandUser = Utils.createElementAndAppendIt('command-user', this.commandRow);
        const promptPath = String(this.commandLine.workingDirectory || '').trim();
        const _promptShell = (() => { try { return localStorage.getItem('beranda-terminal-active-shell') || 'default'; } catch { return 'default'; } })();
        const _promptPrefix = (() => {
            const k = _promptShell.toLowerCase();
            if (k === 'pwsh') return 'PS ';
            if (k === 'powershell') return 'PS ';
            if (k === 'bash' || k === 'git-bash') return '$ ';
            if (k === 'zsh') return '% ';
            if (k === 'fish') return '> ';
            return '';
        })();
        this.commandUser.innerHTML = promptPath ? `${_promptPrefix}${promptPath}>` : `${this.commandLine.username}$`;
        ...
        this.commandEntryKeyDownCallback = (e) => {
            if (e.keyCode == 13) {
                ...
                const entered = String(this.commandEntry.textContent || '');
                this._renderExecutedCommand(entered);
                onEnterCallback(entered);
                return false;
            }
        };
    ...
    _renderExecutedCommand(input) {
        const raw = String(input || '');
        const trimmed = raw.trim();
        if (!trimmed) return;
        const match = raw.match(/^(\s*)(\S+)([\s\S]*)$/);
        if (!match) return;
        const lead = this._escapeInlineHtml(match[1] || '');
        const cmd = this._escapeInlineHtml(match[2] || '');
        const rest = this._escapeInlineHtml(match[3] || '');
        this.commandEntry.innerHTML = `${lead}<span class="cmd-keyword">${cmd}</span>${rest}`;
        Utils.addClass(this.commandRow, 'command-executed');
    }
...
export class CommandLine {
    ...
    constructor(container, username = null) {
        ...
        this.setUsername(username || "NEXA");
        ...
        this.container.addEventListener('click', (e) => {
            ...
            if (e?.target?.closest?.('.nexa-terminal-context-menu, #nexa-terminal-context-menu')) {
                return;
            }
            if (this.commandRow) {
    ...
    find(command) {
        ...
        throw `[${parsedCommand.getCommand()}] Command Not found`;
    }
    ...
    _ansiColor(code, background = false) {
        const fg = {
        ...
        const bg = {
            ...
            104: '#3b8eea', 105: '#d670d6', 106: '#29b8db', 107: '#f2f2f2',
        };
    ...
    _decodeDisplayUrls(message) {
        let s = this._fixTerminalEncoding(String(message ?? ''));
        ...
        s = s.replace(/https?:%2F%2F[^\s)]+/gi, (token) => {
            try {
                return decodeURIComponent(token);
            } catch {
                return token;
    ...
    _renderAnsiToHtml(message) {
        const raw = this._decodeDisplayUrls(String(message ?? ''));
        ...
        let last = 0;
        let open = false;
        const state = { bold: false, italic: false, underline: false, fg: '', bg: '' };
        const closeSpan = () => {
            if (open) {
                out += '</span>';
                open = false;
        ...
        const openSpan = () => {
            const style = this._styleFromAnsiState(state);
            if (style) {
        ...
        while ((match = ansiRegex.exec(raw)) !== null) {
            ...
            if (text) out += this._escapeHtml(text);
            last = ansiRegex.lastIndex;

            const seq = String(match[1] || '0')
                .split(';')
                .map((s) => (s === '' ? 0 : Number(s)))
            ...
            for (let i = 0; i < seq.length; i += 1) {
                ...
                if (code === 0) {
                    ...
                    if (mode === 5) {
                        const idx = seq[i + 2];
                        const color = this._ansi256Color(idx);
                        if (isBg) state.bg = color;
                        else state.fg = color;
                        i += 2;
                    } else if (mode === 2) {
                        const r = seq[i + 2];
                        const g = seq[i + 3];
                        const b = seq[i + 4];
                        const color = this._ansiRgb(r, g, b);
                        if (isBg) state.bg = color;
                        else state.fg = color;
                        i += 4;
                    }
                } else {
                    const fgColor = this._ansiColor(code, false);
                    const bgColor = this._ansiColor(code, true);
                    if (fgColor) state.fg = fgColor;
                    if (bgColor) state.bg = bgColor;
                }
            }
            openSpan();
        }
        ...
        if (tail) out += this._escapeHtml(tail);
        closeSpan();
        return out;
    }
    ...
    output(message) {
        ...
        this.commandRow.setEntry(this._renderAnsiToHtml(message));
        this.commandRow.disable();
    }
    ...
    list(data, cellsPerRow = 4) {
        ...
        let output = '<table class="command-line-table nexa-terminal-table">';
    ...
    listEnhanced(data, cellsPerRow = 4, options = {}) {
        ...
        if (defaultOptions.style === 'list' || defaultOptions.style === 'tree') {
            ...
            output = '<table class="command-line-table nexa-terminal-table' + styleClass + highlightClass + '">';
