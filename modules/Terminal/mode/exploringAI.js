...
const VALIDATION_HANDLERS = {
  ...
  '.js': (name, content, lines, ext) => {
    ...
    // Phase 1b: Strip ES module keywords (export, import) for Function() compatibility
    for (let i = 0; i < currentLines.length; i++) {
      const trimmed = currentLines[i].trimStart();
      // `export` at line start — strip it
      if (trimmed.startsWith('export ')) {
        currentLines[i] = currentLines[i].replace(/^\s*export\s+(default\s+)?/, '');
      }
      // `import` at line start — comment out
      if (trimmed.startsWith('import ')) {
        currentLines[i] = '// ' + currentLines[i];
      }
    }

    // Phase 2: new Function() parsing for remaining errors (V8 stack for line detection)
    const MAX_PASSES = 50;
    ...
    switch (sub) {
                      ...
                      setTimeout(() => {
                        ...
                        if (metricsResult.files.length > 0) {
                          ...
                          for (const f of metricsResult.files.slice(0, 5)) {
                            ...
                            cmd.output('  ' + escHtml(f.path) + ': ' + f.complexity + ' ' + bar + ' (' + f.effective + ' lines)');
                          }
                        }
              ...
              let fixText = '';
                        ...
                        if (fileKeys.length > 0) {
                          ...
                          if (infos2 > 0) {
                            aiCtx += '\nInfo Items:\n';
                            for (const file of fileKeys) {
                              const infosList = staticResult.fileIssues[file].filter(i => i.severity === 'info');
                              if (infosList.length > 0) {
                                const rel = file.replace(scanPath + '/', '');
                                aiCtx += '  ' + rel + '\n';
                                for (const iss of infosList) aiCtx += '    ' + (iss.code || iss.message.substring(0, 40)) + ' at L' + (iss.line || '?') + '\n';
                              }
                            }
                          }
                        }
      ...
      case 'check': {
        ...
        for (const f of metrics.files.slice(0, 15)) {
          ...
          cmd.output(`  ${boldSpan(escHtml(f.path))}`);
          cmd.output(`    Complexity: ${boldSpan(String(f.complexity))} ${complexityBar}`);
          cmd.output(`    Lines: ${f.effective}/${f.lines} | Issues: ${f.issues}`);
        }
