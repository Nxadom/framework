...
function isCodeFile(name) {
  ...
  return codeExts.includes(getExtension(name));
}
...
function analyzerSource(ext) {
  const map = {
    ...
    '.env.example': 'ENV Format Check',
    '.txt': 'Plain Text (skipped)',
    '.rb': 'Ruby Comment-Aware Brace Checker',
    '.go': 'Go Comment-Aware Brace Checker',
    '.rs': 'Rust Comment-Aware Brace Checker',
    '.java': 'Java Comment-Aware Brace Checker',
    '.kt': 'Kotlin Comment-Aware Brace Checker',
    '.swift': 'Swift Comment-Aware Brace Checker',
  };
...
/** Helper: comment-aware brace checking for C-like & Ruby languages */
function commentAwareBraceCheck(ext, content, lines, singleLine, blockOpen, blockClose) {
  const errors = [];
  ...
  for (let i = 0; i < content.length; i++) {
    ...
    // Single-line comment (ends at \n)
    if (!inStr && !inBlockComment && singleLine) {
      if (content.substring(i).startsWith(singleLine)) {
        const nl = content.indexOf('\n', i);
        if (nl >= 0) { i = nl; continue; }
        break;
      }
    }

    // Block comment open/close
    if (!inStr && blockOpen && blockClose) {
      if (!inBlockComment && content.substring(i).startsWith(blockOpen)) {
        const close = content.indexOf(blockClose, i + blockOpen.length);
        if (close >= 0) { i = close + blockClose.length - 1; continue; }
        inBlockComment = true;
        continue;
      }
      if (inBlockComment) {
        if (content.substring(i).startsWith(blockClose)) {
    ...
    if (!inStr) {
      ...
      else if (ch === '[') { bracketCount++; lastBracketLine = lineNum; }
      else if (ch === ']') { bracketCount--; lastBracketLine = lineNum; }
    }
  }

  if (inBlockComment) errors.push(enrichError({ line: lines.length, col: 1, type: 'syntax-error', message: 'Unclosed block comment', severity: 'HIGH', confidence: '100%' }, ext));
  ...
  if (braceCount > 0) errors.push(enrichError({ line: lastBraceLine, col: 1, type: 'syntax-error', message: `Unclosed { \u2014 ${braceCount} not closed`, severity: 'HIGH', confidence: '100%' }, ext));
  ...
  if (parenCount > 0) errors.push(enrichError({ line: lastParenLine, col: 1, type: 'syntax-error', message: `Unclosed ( \u2014 ${parenCount} not closed`, severity: 'HIGH', confidence: '100%' }, ext));
  ...
  if (bracketCount > 0) errors.push(enrichError({ line: lastBracketLine, col: 1, type: 'syntax-error', message: `Unclosed [ \u2014 ${bracketCount} not closed`, severity: 'HIGH', confidence: '100%' }, ext));
...
const VALIDATION_HANDLERS = {
  ...
  '.js': (name, content, lines, ext) => {
    ...
    // Replaces bad "function1" with valid "function" per line
    // Only flags when keyword is glued to DIGITS (e.g. function1, class2),
    // NOT when glued to letters (e.g. classList, forEach, constructor)
    let currentLines = lines.map(l => l);
    const kwRe = /\b(function|class|var|let|const|if|for|while|switch|catch|return|throw|import|export)(\d+)/g;
  ...
  '.html': (name, content, lines, ext) => {
    ...
    while ((match = tagRe.exec(content)) !== null) {
      ...
      // HTML void elements — never need closing
      ...
      if (isClosing) {
        ...
        // Self-closing tag <tag .../> — skip (valid SVG/HTML syntax)
        continue;
      } else {
        // Opening tag <tag ...>
  ...
  '.env.example': (n, c, l, e) => VALIDATION_HANDLERS['.env'](n, c, l, '.env'),

  '.txt': (name, content, lines, ext) => {
    // Plain text — no syntax validation needed
    return [];
  },

  '.rb': (name, content, lines, ext) => {
    // Ruby: # single-line comments, no block comments, check braces/parens/strings
    return commentAwareBraceCheck('.rb', content, lines, '#');
  },
  '.go': (n, c, l, e) => commentAwareBraceCheck('.go', c, l, '//', '/*', '*/'),
  '.rs': (n, c, l, e) => commentAwareBraceCheck('.rs', c, l, '//', '/*', '*/'),
  '.java': (n, c, l, e) => commentAwareBraceCheck('.java', c, l, '//', '/*', '*/'),
  '.kt': (n, c, l, e) => commentAwareBraceCheck('.kt', c, l, '//', '/*', '*/'),
  '.swift': (n, c, l, e) => commentAwareBraceCheck('.swift', c, l, '//', '/*', '*/'),
    ...
    switch (sub) {
      ...
      case '': {
        ...
        const summaryTable = new TabelRaw([
          ...
          ...(staticResult.summary.warnings > 0 ? [{ Metric: 'Static Issues', Value: String(staticResult.summary.warnings) }] : []),
        ...
        setTimeout(() => {
          // Step 2: Static Issues Overview (hanya warning, bukan info)
          if (fileKeys.length > 0 && staticResult.summary.warnings > 0) {
            cmd.info('⚠️  Static Issues (' + staticResult.summary.warnings + ' warnings):');
            const sortedFiles = fileKeys.sort((a, b) => {
              const aWarn = staticResult.fileIssues[b].filter(i => i.severity !== 'info').length;
              const bWarn = staticResult.fileIssues[a].filter(i => i.severity !== 'info').length;
              return aWarn - bWarn;
            });
            for (const file of sortedFiles.slice(0, 5)) {
              const issues = staticResult.fileIssues[file].filter(i => i.severity !== 'info');
          ...
          setTimeout(() => {
              ...
              setTimeout(() => {
                ...
                setTimeout(() => {
                  ...
                  cmd.output('Syntax Errors: ' + (totalErrors || 0) + ' | Warnings: ' + totalWarnings);
                  ...
                  setTimeout(() => {
                    // Step 6: Files Requiring Attention (hanya warning/error, bukan info)
                    if (allAffected.length > 0) {
                      let hasNonInfo = false;
                      for (const rel of allAffected) {
                        const fileIssues = affectedFileIssues[rel] || [];
                        const nonInfo = fileIssues.filter(fi => fi.severity !== 'INFO');
                        if (nonInfo.length > 0) { hasNonInfo = true; break; }
                      }
                      if (hasNonInfo) {
                    ...
                    setTimeout(() => {
                      // Step 7: Debug Priority (skip info-only)
                      const nonInfoPriority = priorityList.filter(p => p.priority < 2 || (p.detail && !p.detail.includes('info item')));
                      if (nonInfoPriority.length > 0) {
                        cmd.info('🔥  Debug Priority');
                        let rank = 1;
                        for (const p of nonInfoPriority) {
                      ...
                      setTimeout(() => {
                        ...
                        let aiCtx = '── Debug Summary ──\n\n';
                        aiCtx += 'Directory: ' + scanPath + '\n';
          ...
          for (const f of errFileKeys) {
            ...
            for (const r of rootErrs) {
              ...
              let fixText = '';
                        ...
                        // ── Simpan ke NXUI.ref('debug') ──
                        void (async () => {
                          try {
                            const nx = globalThis.NXUI;
                            if (!nx?.ref?.set) return;
                            const dirName = String(scanPath || '').replace(/\\/g, '/').split('/').filter(Boolean).pop() || 'unknown';
                            const debugId = 'debug_' + dirName;
                            const filesWithErrors = errFileKeys.map((f) => {
                              const entry = errorsResult[f];
                              const rel = f.replace(scanPath + '/', '').replace(/\\/g, '/');
                              const errs = (entry?.errors || []).map((r) => ({
                                line: r.line,
                                col: r.col || 1,
                                type: r.errorType || r.type || 'Syntax Error',
                                message: r.message || '',
                                severity: r.severity || 'HIGH',
                                suggestion: r.suggestion || '',
                              }));
                              return { file: rel, errors: errs };
                            });
                            await nx.ref.set('debug', {
                              id: debugId,
                              directory: String(scanPath || ''),
                              directoryName: dirName,
                              totalErrors: totalErrors || 0,
                              totalWarnings: totalWarnings || 0,
                              filesWithErrors,
                              errorFileCount: errFileKeys.length,
                              timestamp: Date.now(),
                            });
                            window.dispatchEvent(new CustomEvent('nexa-debug-updated', {
                              detail: { id: debugId, totalErrors: totalErrors || 0, errorFileCount: errFileKeys.length },
                            }));
                          } catch (_) {}
                        })();

                        // ── Auto-inject ke composer chat (hanya jika ada error) ──
                        if ((totalErrors || 0) > 0) {
                          try {
                            const cleanCtx = aiCtx
                              .replace(/^──\s*Debug Summary\s*──[\s\S]*?\n\n/, '')
                              .replace(/\nEnd Summary\s*$/, '')
                              .trim();
                            if (cleanCtx) {
                              window.dispatchEvent(new CustomEvent('nexa-debug-to-chat', {
                                detail: { text: cleanCtx, directory: String(scanPath || '') },
                              }));
                            }
                          } catch (_) {}
                        }
